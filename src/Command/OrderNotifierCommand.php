...
use App\Entity\PurchasingOrderInvoiceTax;
...
class OrderNotifierCommand extends Command
{
  ...
  private function getCloseAnalysisTasksOrders(int $limit, int $datelimit = null): ?array
  {
    ...
    $tasks = $this->em->getRepository(Task::class)
      ...
      ->innerJoin('\App\Entity\Category', 'C', 'WITH', 'T.category = C.id')
  ...
  protected function getGenerateCommissionOrders(int $limit, int $datelimit = null): ?array
  {
    $salesOrders = $this->em->getRepository(SalesOrder::class)
      ...
      ->innerJoin('\App\Entity\PurchasingOrder', 'PO', 'WITH', 'PO.mainOrder = O.id AND PO.orderType =:pOrderType')
    ...
    if (!$carrierInvoiceTax) {
        ...
        $purchasingOrderTax = new \App\Entity\PurchasingOrderInvoiceTax();
  ...
  private function _getInvoiceOrdersTemplateParams(Order $order): array
  {
    ...
    return [
      ...
      'app_domain'      => 'https://app.controleonline.com',
  ...
  private function getMainOrderDiscoveryOrders(int $limit, int $datelimit = null): ?array
  {
    ...
    $salesOrder =  $this->em->getRepository(SalesOrder::class)
      ->createQueryBuilder('O')
      ->select()
      ->innerJoin('\App\Entity\PurchasingOrderInvoiceTax', 'OIT', 'WITH', 'OIT.order = O.id')
      ->innerJoin('\App\Entity\PurchasingOrderInvoiceTax', 'OITS', 'WITH', 'OITS.order = O.id')
      ->leftJoin('\App\Entity\PurchasingOrder', 'PO', 'WITH', 'PO.mainOrder = O.id')
  ...
  private function _getOrdersTemplateParams(Order $order): array
  {
    ...
    return [
      ...
      'api_domain'     => 'https://' . isset($_SERVER) && isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'api.controleonline.com',
      'app_domain'     => 'https://app.controleonline.com',
