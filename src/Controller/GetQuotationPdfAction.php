...
class GetQuotationPdfAction
{
    ...
    private function getBody(Quotation $data)
    {
        $config = $this->manager->getRepository(Config::class)->findOneBy([
            'people' => $this->manager->getRepository(PeopleDomain::class)->findOneBy(['domain' => $_SERVER['HTTP_HOST']])->getPeople(),
        ...
        $body = preg_replace(
            '/\\{\\{\\s+app_link\\s+\\}\\}/',
            $protocol . str_replace('api', 'app', $_SERVER['HTTP_HOST']),
            $body
        );
