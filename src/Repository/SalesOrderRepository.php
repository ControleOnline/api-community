...
class SalesOrderRepository extends ServiceEntityRepository
{
    ...
    public function getByProspectContacts(int $limit = 10): array
    {
      $sql = "
        ...
        FROM orders ord
          ...
          INNER JOIN config pc ON
              pc.people_id = ord.provider_id

        WHERE
          ord.notified            = 0
          AND pc.config_key LIKE 'mautic-%'
          AND ord.status_id = 1
    ...
    public function updateOrderTotalFromInvoicesPrice(int $orderId): void
    {
      $conn = $this->getEntityManager()->getConnection();

      // get total price invoices

      $price = 0;
      $SQL   = <<<SQL
        select sum(invoice.price) as total
        from invoice
          inner join order_invoice on order_invoice.invoice_id = invoice.id
